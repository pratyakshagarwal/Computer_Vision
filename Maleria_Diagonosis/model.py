...
class CNN_Model(tf.keras.models.Model):
    def __init__(self, dropout_rate=0.2, optimizer=Adam(learning_rate=Configurations['learning_rate']), loss_fn=BinaryCrossentropy(), metric=CustomAccuracy(), customly=False):
        ...
        self.model.add(Dense(Configurations['num_classes'], activation='sigmoid'))
    ...
    def train_nd_evaluate(self, train_data, val_data=None, epochs=Configurations['n_epochs'], batch_size=Configurations['batch_size'], verbose=1, callbacks=LossCallback()):
        if self.customly:
            ...
            if val_data != None:
                self.model.fit(train_data, validation_data=val_data, epochs=epochs, batch_size=batch_size, verbose=verbose, callbacks=callbacks)
            else :
                self.model.fit(train_data, validation_split=0.2, epochs=epochs, batch_size=batch_size, verbose=verbose, callbacks=callbacks)
